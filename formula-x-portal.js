...
async function staffForCurrentUser(){
  ...
  if(userError){
    console.error('Formula X V31 - auth.getUser:',userError);
  ...
  if(error){
    console.error('Formula X V31 - get_my_staff_profile:',error);
...
async function openPortal(){
  try{
    ...
    console.error('Formula X V31 - acces profesor:',e);
...
async function loadAttendance(){
  ...
  if(error){
    console.error('Formula X V31 - attendance:',error);
...
function renderAttendance(){
  ...
  for(const r of arr){
    h+=`<tr><td><div class="name">${esc(r.full_name)}</div><span class="attendance-student-email">${esc(r.email||'')}</span></td><td class="attendance-total" data-attendance-total="${esc(r.id)}">${attendanceCountForStudent(r.id)}/17</td>`;
...
async function saveAttendance(input){
  ...
  try{
    ...
    console.error('Formula X V31 - save attendance:',error);
...
function renderActivity(){
  ...
  const labels={student_added:'Elev adăugat',student_updated:'Elev actualizat',student_archived:'Elev arhivat',student_restored:'Elev restaurat',group_created:'Grupă creată',group_updated:'Grupă actualizată',group_deleted:'Grupă ștearsă',group_members_updated:'Componență grupă schimbată',session_created:'Ședință programată',session_status:'Status ședință schimbat',session_deleted:'Ședință ștearsă'};
...
;(async()=>{
  ...
  try{
    ...
    console.error('Formula X V31 - bootstrap:',error);
