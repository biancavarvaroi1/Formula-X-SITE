...
async function loadAttendance(){
  ...
  const {data,error}=await sb.from('student_attendance').select('enrollment_id,week_number,updated_at').order('week_number',{ascending:true});
  if(error){
...
async function loadPageViews(){
  ...
  if(error){
    console.error('Formula X V30 - analytics:',error);
    pageViews=[];
...
function renderAttendance(){
  ...
  if(!attendanceReady){root.innerHTML='<div class="attendance-setup"><strong>Prezența nu este încă activată în Supabase.</strong><br>Rulează o singură dată fișierul <code>sql/05-ATTENDANCE-V31.sql</code>, apoi apasă „Actualizează”.</div>';return}
...
async function saveAttendance(input){
  ...
  try{
    ...
    const total=document.querySelector(`[data-attendance-total="${CSS.escape(enrollmentId)}"]`);
...
async function saveNewStudent(){
  ...
  const {data,error}=await sb.from('enrollments').insert(payload).select('id').single();if(error){console.error(error);alert('Elevul nu a putut fi adăugat. Verifică dacă ai rulat SQL-ul V32.');return}
...
async function saveStudent(id){
  const r=students.find(x=>x.id===id);if(!r)return;
  const email=$('mEmail').value.trim().toLowerCase();const payload={full_name:$('mName').value.trim(),email:email||null,phone:$('mPhone').value.trim()||null,level:$('mLevel').value.trim()||null,status:$('mStatus').value,teacher_note:$('mTeacherNote').value.trim()||null};
...
async function rotateGroupInvite(id){
  ...
  const {error}=await sb.from('teacher_groups').update({join_token:newToken,join_enabled:true}).eq('id',id);
  if(error){console.error(error);alert('Nu am putut genera linkul nou.');return}
...
$('loginForm').addEventListener('submit',async e=>{
  ...
  if(error){
    console.error('Formula X V30 - login:',error);
...
async function logoutProfessor(){
  ...
  if(error){
    console.error('Formula X V30 - logout:',error);
...
;(async()=>{
  ...
  try{
    ...
    console.error('Formula X V32.1 - bootstrap:',error);
    const code=String(error?.message||error||'');
    if(code.includes('TIMEOUT')) showLogin('Portalul nu a răspuns la timp. Reîncarcă pagina; dacă problema continuă, verifică publicarea fișierelor V32.1 în GitHub.');
