(() => {
  ...
  async function init(){
    if(!window.supabase?.createClient || !uuid.test(token)){failLink();return}
    ...
    window.fxGroupDb=db;
    const {data,error}=await db.rpc('fx32_group_info',{p_token:token});
    if(error){console.error('Formula X V32 group info:',error);failLink();return}
  ...
  $('joinForm').addEventListener('submit',async event=>{
    ...
    try{
      const {data,error}=await window.fxGroupDb.rpc('fx32_join_group',{p_token:token,p_full_name:name,p_phone:phone,p_website:website});
      ...
      console.error('Formula X V32 join group:',error);
